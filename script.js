...
──────────────────────────────────────────────────────────*/
 window.openTab = (evt, id) => {
  ...
  // 🔁 Cas : on clique sur l'onglet déjà actif
  if (isAlreadyActive) {
    const isAtTop = window.scrollY <= 10;
    if (isAtTop) {
      // 🟢 Déjà en haut → recharger toute la page
      location.reload();
    } else {
      // 🔼 Scroll vers le haut + shuffle si on est sur "home"
    ...
    return; // ⛔️ Ne pas continuer : pas de changement d’onglet
  }

  // 🆕 Cas : on change d’onglet → juste activer le nouvel onglet
...
// Ajouter l'événement popstate pour gérer le bouton retour
...
window.addEventListener('load', () => {
  ...
  // ✅ On NE FAIT RIEN D’AUTRE ! Ni shuffle, ni render ici.
  // Pas de renderFeed(), pas de shuffleFeed()
  // → L’Accueil ne doit jamais être modifié juste en y revenant
};
...
function updateFollowBtn(uid, b) {
  // Sécurité si followingIds pas encore chargé
  if (!followingIds || typeof followingIds.has !== 'function') {
    b.textContent = '...';
    b.disabled = true;
    return;
  }
...
async function toggleFollow(uid, b) {
  ...
  if (on) {
    ...
    await Promise.all([
      setDoc(myRef, {}),
      setDoc(hisRef, { uid: currentUser.uid, date: serverTimestamp() })
    ]);
    followingIds.add(uid);
  }

  updateFollowBtn(uid, b);
  refreshFollowerCounters(uid);
}

async function refreshFollowerCounters(uid) {
  const snap = await getDocs(collection(db, 'follows', uid, 'list'));
  const n = snap.size;
  document.querySelectorAll(`[data-followers-of="${uid}"]`)
    .forEach(el => el.textContent = `Followers : ${fmt(n)}`);
  }
/*──────────────────────────────────────────────────────────
  7. WALLET dépôt
──────────────────────────────────────────────────────────*/
...
async function deposit(amount) {
  ...
  if (!Number.isFinite(amount) || amount < 2000)
    return alert("Montant minimum de dépôt : 2 000 FCFA");
...
async function renderFeed() {
  ...
  // BOOSTED POSTS (affichées même si +7 jours, tant que le boost est actif)
  const qBoost = query(
    collection(db, 'posts'),
    where('boosted', '>', 0),
  ...
  // FOLLOWED USERS : on affiche seulement les posts récents (-7 jours)
...
function renderVideoFeed() {
  ...
  onSnapshot(
    query(
      collection(db, 'posts'),
      where('videoUrl', '!=', null),
      where('boosted', '>', 0), // 👉 Affiche uniquement les vidéos boostées
      orderBy('createdAt', 'desc')
    ),
    snap => {
      c.innerHTML = '';
      ...
      // Optionnel : Mélanger aléatoirement
...
// Vérifie si l'utilisateur est certifié (collection 'verified')
...
// Temps relatif
  ...
  // Étiquette sponsorisé si boosté
  if ((d.boosted || 0) > 0) {
  ...
  if (d.videoUrl) {
    ...
    const vidHtml = `
      ...
      ${d.videoTitle ? `<div style="font-weight:bold;margin-top:4px;">${linkifyText(d.videoTitle)}</div>` : ''}
    `;
    cont.innerHTML = vidHtml;
  } else if (d.text) {
    // Texte + liens
...
like.className = 'fa-regular fa-heart'; // cœur vide au départ
...
// Vérifie si déjà liké depuis Firestore
if (currentUser) {
  const likeRef = doc(db, 'posts', id, 'likes', currentUser.uid);
  getDoc(likeRef).then(snap => {
...
act.append(like, lCnt);
  // 🟡 EDIT (uniquement mes posts) — placé AVANT commentaires
  if (isMine) {
...
async function editPost(postId, data) {
  ...
  // Si rien changé, on ne fait rien
  ...
  try {
    ...
    console.error('Erreur mise à jour post', e);
...
async function loadComments(pid, container, cnt) {
  ...
  s.forEach(d => {
    ...
    username.onclick = () => openUserFeed(c.userId);
...
async function prepareWallet() {
  ...
  if (!walletSnap.exists()) {
    // Créer un wallet vide avec 0 FCFA
    await setDoc(walletRef, { balance: 0 });
    console.log("Wallet initialisé à 0 FCFA");
  }
}
...
window.boostPost=async()=>{
  ...
  const cost=Math.ceil(v/1000)*500;if(!(await debit(cost)))return;
  try{
await updateDoc(doc(db, 'posts', boostId), {
  boosted: increment(v),
  views: increment(v),
  boostAt: serverTimestamp() // ← très important pour savoir quand le boost a commencé
});
    qs('boostStatus').textContent=`Boost succès ✔️`;qs('boostViews').value='';boostId='';renderFeed();renderMyFeed();await loadTotalViewsAndBalance();
...
async function loadTotalViewsAndBalance(period = 'all') {
  ...
  // Ajoute filtre période si besoin
...
snap.forEach(d => {
  ...
  totalViews += views + boosted + vuesParLike;
});
...
const raw = Math.floor(totalViews / 1000) * 150;
  ...
  currentBalance = potential; // mise à jour interne seulement
}
// 🔁 Fonction de retrait
window.withdrawBalance = async () => {
  ...
  if (!phone || phone.length < 8) {
    return alert("Veuillez entrer un numéro valide.");
  }

  // Recalculer le solde si jamais la page n’est pas fraîche
  await loadTotalViewsAndBalance(); // pour éviter erreurs
  ...
  try {
    ...
    // 🖥️ Réinitialiser l’UI
    qs("withdrawAmount").value = "";
    qs("withdrawPhone").value = "";

    // 🔁 Recharger le solde à l’écran
    await loadTotalViewsAndBalance();

    alert("Demande de retrait enregistrée et transmise !");
  } catch (err) {
...
window.signupWithEmail = async () => {
  ...
  if (age < 18) return alert('Âge min 18');
  if (p.length < 6) return alert('Mot de passe ≥6');
  ...
  try {
    ...
    // ✅ Suivi automatique de l'admin (mboaskill)
    const mboaskillUID = "RyYsQko6cLSltTL0uiUYRx0guag1";
    await Promise.all([
      setDoc(doc(db, "follows", user.uid, "list", mboaskillUID), {}),
      setDoc(doc(db, "follows", mboaskillUID, "list", user.uid), {
        uid: user.uid,
        date: serverTimestamp()
      })
    ]);

    // ✅ Met à jour followingIds pour que les boutons soient corrects
    await loadFollowing();
...
document.getElementById("notchpayBtn")?.addEventListener("click", async () => {
  const m = parseFloat(qs("montant").value);
  if (isNaN(m) || m < 2000) return alert("Min 2 000 FCFA");
  if (!currentUser) return alert("Connectez-vous");

  await prepareWallet(); // ← ici on initialise le wallet si nécessaire
...
// Vérifie si l’URL est de la forme https://mboaskill.cm/u/<uid>
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (match && match[1]) {
    ...
    if (typeof window.openUserFeed === 'function') {
      ...
      // Si openUserFeed n'est pas encore dispo, on attend
    ...
    // Nettoyer l’URL visuellement
    history.replaceState({}, '', '/');
  }
});
let suggestionInterval; // à placer en global
...
async function loadSuggestions() {
  ...
  suggestionsList.textContent = 'Chargement…';
  ...
  // Fonction pour mélanger et afficher
  ...
  updateSuggestions(); // affiche une fois immédiatement
...
window.certifyUser = async function () {
  ...
  try {
    ...
    if (balance >= 18000) {
      // Déduction de 18 000 Fcfa
      await updateDoc(walletRef, {
        balance: balance - 18000
      });

      // Mise à jour du statut certifié
      await setDoc(verifiedRef, {
        status: true
      });

      alert("✅ Vous êtes maintenant certifié !");
    } else {
      // Ne pas déduire, mettre status à false
      await setDoc(verifiedRef, {
        status: false
      });

      alert("💸 Solde insuffisant. Le montant requis est de 18 000 Fcfa.");
    }
  } catch (error) {
...
async function publishMusic() {
  ...
  try {
    ...
    if (!walletSnap.exists()) {
      return alert("Aucun portefeuille trouvé.");
    }
    ...
    // ✅ Déduire 5100
    await updateDoc(walletRef, {
      balance: currentBalance - 5100
    });

    statusMsg.textContent = "Téléchargement du fichier audio...";
    ...
    statusMsg.textContent = "✅ Musique publiée avec succès.";
    document.getElementById('nom').value = '';
    document.getElementById('titre').value = '';
    document.getElementById('audioFile').value = '';
  } catch (err) {
    console.error("Erreur lors de la publication :", err);
    statusMsg.textContent = "❌ Erreur lors de la publication.";
  }
}
async function renderMusicFeed() {
  ...
  try {
    ...
    for (const docSnap of snapshot.docs) {
      ...
      // 🎯 1. Ne pas afficher les musiques expirées (10 jours)
      ...
      // 🎯 2. Vérifie si l’utilisateur a déjà écouté
      ...
      // 🎯 3. Si première écoute : déduction + ajout des vues
      if (user && !alreadyViewed && user.uid !== d.userId) {
        ...
        if (balance >= 55) {
          ...
          // ➕ Ajouter 340 vues dans totalviewcontent
          const statsRef = doc(db, 'totalviewcontent', d.userId);
          await setDoc(statsRef, { views: increment(340) }, { merge: true });

          // ➕ Ajouter 340 vues dans totalviews (global)
          const globalViewsRef = doc(db, 'totalviews', d.userId);
          await setDoc(globalViewsRef, { views: increment(340) }, { merge: true });
        } else {
          alert("Solde insuffisant pour écouter cette musique.");
        }
      }

      // 🎯 4. Affiche le bloc de musique
      const div = document.createElement('div');
